...
const IssueUpdatesList = () => {
  ...
  useEffect(() => {
    const fetchUserUpdates = async () => {
      try {
        ...
        const response = await axios.get(`https://labrequirement.onrender.com/api/auth/issueupdates`, {
  ...
  if (loading) return <div className="invcontainer">Loading updates...</div>;
  ...
  return (
    <div className="invcontainer">
      ...
      {updates.length > 0 ? (
        ...
        <p>No updates available for your issues</p>
      )}
    </div>
  );
};
