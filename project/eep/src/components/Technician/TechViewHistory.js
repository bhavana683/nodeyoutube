...
const TechViewHistory = () => {
  ...
  const handleAccept = async (issueId) => {
    try {
      await axios.post(`https://labrequirement.onrender.com/api/auth/acceptissue/${issueId}`, {}, {
  ...
  useEffect(() => {
    const fetchIssues = async () => {
      try {
        const response = await axios.get('https://labrequirement.onrender.com/api/auth/pending', {
          withCredentials: true
        });
